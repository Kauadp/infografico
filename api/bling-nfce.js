...
async function fazerRequisicao(url, maxRetries = 3, retryDelay = 1000) {
  for (let tentativa = 1; tentativa <= maxRetries; tentativa++) {
    try {
      ...
      if (res.status === 429) {
        const waitTime = retryDelay * Math.pow(2, tentativa - 1); // Backoff exponencial
...
async function buscarNotas(dataInicio, dataFim, limite = 100) {
  ...
  while (hasMore && allNotas.length < limite) {
    ...
    // Pausa mínima para respeitar o limite de 3 req/s
    if (page > 1) await new Promise(r => setTimeout(r, 350)); // ~333ms entre requisições
  }
...
function processarDados(notas, incluirClientes = false) {
  ...
  notas.forEach(nota => {
    if (!nota || nota.situacao !== 5) return;
  ...
  try {
    ...
    for (let i = 0; i < notas.length; i += batchSize) {
      ...
      if (i + batchSize < notas.length) {
        await new Promise(r => setTimeout(r, 350)); // Pausa entre lotes
      }
    }
